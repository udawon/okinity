...
function TimelineCard({ q, prices }: { q: Inquiry; prices: TourPrices }) {
  ...
  return (
    <div
      ...
      className="min-w-[150px] flex-1 cursor-grab rounded-lg border border-line bg-surface p-2 shadow-sm active:cursor-grabbing"
    ...
    >
      ...
      <p className="mt-0.5 text-[11px] text-muted">
        ...
        {q.time ? ` · 희망 ${q.time}` : ''}
        {q.scheduledTime ? ` · 배정 ${q.scheduledTime}` : ''}
      </p>
        ...
        <div className="flex items-center justify-between border-b border-line bg-surface px-5 py-3">
          <div>
            ...
            <p className="text-xs text-muted">
              카드를 시간대로 끌어 배치하세요. 같은 시간에 겹치면 표시됩니다.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="max-h-[75vh] overflow-y-auto p-4">
          ...
          >
            <p className="mb-2 text-xs font-semibold text-muted">
              시간 미배정 ({unscheduled.length}) — 여기로 끌어 시간 해제
            </p>
            {unscheduled.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {unscheduled.map((q) => (
                  <TimelineCard key={q.id} q={q} prices={prices} />
                ))}
              </div>
            )}
          </div>
            ...
            {HOURS.map((h) => {
              ...
              return (
                <div
                  key={h}
                  {...drop(slot(h))}
                  className={`flex gap-3 px-3 py-2 ${overlap ? 'bg-amber-50' : ''}`}
                >
